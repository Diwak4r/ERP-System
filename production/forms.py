...
class ProductionEntryForm(forms.ModelForm):
    def __init__(
        ...
        *args,
        ...
        worker_choices: list[tuple] | None = None,
        item_choices: list[tuple] | None = None,
        ...
        **kwargs
        ...
        # Always set queryset for validation, and use choices if provided for rendering efficiency
    ...
    def _hydrate_targets(self) -> None:
        ...
        if self.target_rules_cache is not None:
            rule = self.target_rules_cache.get(item.id if hasattr(item, "id") else item)
        else:
            rule = (
...
class BaseProductionEntryFormSet(forms.BaseFormSet):
    def __init__(self, *args, **kwargs):
        ...
        if section:
            ...
            # We only need the latest rule for each item
